...
Pick `grafana` or `elasticsearch` when you need an interactive
...
key the way the `elasticsearch` scenario's ES output does), so a
record looks like:
...
Use `scripts/gcs-gather.py` to pull a time window of RRPairs and assemble
...
- Shells out to `gcloud storage ls` / `cat`, so it inherits whatever GCP
  credentials the caller already has (ADC, `gcloud auth login`, Workload
  Identity). No Python SDK dependency.
- Streams each gzipped NDJSON object, filters records in-memory, strips
  the internal OTLP envelope, backfills `cluster` from the OTLP
  resource attribute.
...
- **OTel Collector contrib `0.108.0`** — used identically by `grafana` and
